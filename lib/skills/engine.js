...
const defaultState = Object.freeze({
  alma: 30, // Alma atual
  almaMax: 30, // Alma máxima
  maxVitalidade: 100, // Vitalidade Máxima (do personagem)
  vitalidadeAtual: 100, // Vitalidade atual
...
function normalizeState(state) {
  ...
  s.almaMax = Math.max(0, toIntNonNeg(state?.almaMax ?? s.almaMax));
  s.alma = toIntNonNeg(s.alma);
  if (s.almaMax > 0) s.alma = Math.min(s.alma, s.almaMax);
  s.maxVitalidade = Math.max(1, toIntNonNeg(s.maxVitalidade) || 1);
  s.vitalidadeAtual = toIntNonNeg(state?.vitalidadeAtual ?? s.vitalidadeAtual);
  s.vitalidadeAtual = Math.min(s.vitalidadeAtual, s.maxVitalidade);
...
const SkillEngine = (() => {
  ...
  function setState(partial) {
    ...
    saveState(_state);
    // Dispara evento para sincronização em tempo real
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('skills_state_changed', { detail: _state }));
    }
    return getState();
  }
  ...
  // Recarrega o estado do localStorage (usado quando alterado externamente)
  ...
  // Bônus de Defesa enquanto ativo: +1/4 da Alma MÁXIMA (dinâmico)
  function getAdditionalDefense() {
    if (_state.insano.activeRounds > 0) {
      return Math.floor((_state.almaMax || 0) * 0.25);
    }
    return 0;
  }
  ...
  function processIncomingPhysical({ damage, isBlockSuccess = false, targetMaxVitalidade }) {
    ...
    if (isBlockSuccess) {
      ...
      const nextAlma = Math.max(0, before.alma + almaHeal);
      _state.alma = Math.min(nextAlma, before.almaMax);
    }
